...
from Dash2.cyber_physical_operator.nurse01 import Nurse
...
def test_num_computers(hosts=None, num_trials=3, independent=['num_computers', Range(5, 21, 5)]):
    exp = Experiment(NurseTrial,
                     hosts=hosts,
                     exp_data={'num_nurses': 1, 'num_patients': 1, 'num_medications': 1, 'timeout': 0},  # was 20
...
def test_timeout():
    exp = Experiment(NurseTrial,
                     exp_data={'num_nurses': 1, 'num_patients': 5, 'num_medications': 10, 'num_computers': 20},
                     num_trials=8)
...
if __name__ == "__main__":
    exp, results = run_one(sys.argv[1:], num_trials=1, max_iterations=1, independent=['num_computers', Range(4,20)])
